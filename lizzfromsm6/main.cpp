/**
 ...
 This program can add, subtract, multiply, and divide numbers, 
 as well as store and retrieve variable values. 
It accepts a sequence of commands from the file "TestCase.txt"
and outputs the reponses to those commands to the console.
The program accepts 5 basic types of commands, listed below.
...
// lizz-defined
bool isAssign(const char* op);
bool isAdd(const char* op);
bool isSub(const char* op);
bool isMult(const char* op);
bool isDiv(const char* op);
bool isInc(const char* op);
bool isDec(const char* op);
bool isAddAssign(const char* op);
bool isSubAssign(const char* op);
bool isMultAssign(const char* op);
bool isDivAssign(const char* op);




// 
// Return whether the given character is a char
//
bool isChar(char c)
{
  if (c >= 'a' && c <= 'z') return true;
  else if (c >= 'A' && c <= 'Z') return true;
  else return c == '_'; // uhh is this a booL??
}
...
bool isDigit(char c)
{
  ...
  else return c == '.'; // why is THIS  a bool?
}


// 
// Return whether the given character is an operator
//
bool isOperator(char c)
{
  /*
...
//
// Return whwether the given character is a space (or tab or new line...)
//
bool isSpace(char c)
{
  /*
...
//isSetEqual
bool isAssign(const char* op) { return (strcmp(op, "=") == 0); }
bool isAdd(const char* op) { return (strcmp(op, "+") == 0); }
bool isSub(const char* op) { return (strcmp(op, "-") == 0); }
bool isMult(const char* op) { return (strcmp(op, "*") == 0); }
bool isDiv(const char* op) { return (strcmp(op, "/") == 0); }
bool isInc(const char* op) { return (strcmp(op, "++") == 0); }
bool isDec(const char* op) { return (strcmp(op, "--") == 0); }
bool isAddAssign(const char* op) { return (strcmp(op, "+=") == 0); }
bool isSubAssign(const char* op) { return (strcmp(op, "-=") == 0); }
bool isMultAssign(const char* op) { return (strcmp(op, "*=") == 0); }
bool isDivAssign(const char* op) { return (strcmp(op, "/=") == 0); }
...
op_t recognizeOp(const char* op)
{
  ...
  if (strcmp(op, "=") == 0) return ASN;
    ...
    return BAD_OP;
  */
  if (isAssign(op))     return ASN;
  if (isAdd(op))        return ADD;
  if (isSub(op))        return SUB;
  if (isMult(op))       return MULT;
  if (isDiv(op))        return DIV;
  if (isInc(op))        return INC;
  if (isDec(op))        return DEC;
  if (isAddAssign(op))  return ADDASN;
  if (isSubAssign(op))  return SUBASN;
  if (isMultAssign(op)) return MULTASN;
  if (isDivAssign(op))  return DIVASN;
