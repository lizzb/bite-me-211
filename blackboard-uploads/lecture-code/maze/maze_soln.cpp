...
/**
 ...
 @param (output parameter) the sequence of moves required to move to the Maze exit
 @return whether the solution attempt was successful
 */
bool Maze::solve(deque<direction_t>& solution)
{
  deque<direction_t> moves;
  bool up, down, left, right;
  direction_t next;

  do
  {
    getMoves(up, right, left, down);
    if (up)
    {
      moves.push_back(UP);
      move(UP);
    ...
    else //Current move is unavailable
    {
#ifdef DEBUG
cout << "Dead end:" << *this << "\nStack: " << moves << endl;
system("pause");
...
/**
 Example function to show how solution could be computed recursively
 Rather than return the sequence of moves, this solution marks only 
   the solution in its traversal
 Because each function call keeps track of which directions it has
   already tried, this strategy would work even if getMoves didn't
   eliminate previously-explored hallways from consideration

 @return whether it is possible for the walker to reach the exit from
         its current location
 */
bool Maze::solve_recursive()
{
  bool up, down, left, right;
  ...
  {
    move(UP);
    //If we're at the exit, we've finished
    //If not, recursively explore the new location and return true if successful
    if (isSolved() || solve_recursive()) 
      return true;
    //If recursive call failed, backtrack and try next direction
    clearVisited(location); //Any paths that do not go to the solution will be cleared
    move_back(UP);
  }
  if (right)
  {
  ...
  //If we've tried (and failed) all possible moves, this is a dead end
#ifdef DEBUG
cout << "Dead end:  " << *this << endl;
system("pause");
#endif
  return false;
}
